...
function AttackOnlyGroups( enemy, currentRun )
	...
	while IsAIActive( enemy, currentRun ) do
		...
		enemy.AINotifyName = "WithinDistance"..enemy.ObjectId
		NotifyWithinDistanceAny({ Id = searchId, DestinationNames = enemy.TargetGroups, Distance = aiData.AIAttackDistance, ScaleY = aiData.AttackDistanceScaleY or 0.5, Notify = enemy.AINotifyName, UseLocationZ = true })
		waitUntil( enemy.AINotifyName )
...
function PassiveAttack( enemy, currentRun )
	...
	while IsAIActive( enemy, currentRun ) do
		if weaponAIData.AIResetDistance ~= nil then
			...
			NotifyWithinDistanceAny({ Id = enemy.ObjectId, DestinationNames = weaponAIData.TargetGroups, Distance = weaponAIData.AIAttackDistance, ScaleY = 0.5, Notify = enemy.AINotifyName, Timeout = 0.1 })
			waitUntil( enemy.AINotifyName )
			local nearbyTargetId = NotifyResultsTable[enemy.AINotifyName]
...
function RemoteAttack( enemy, currentRun )
	...
	while IsAIActive( enemy, currentRun ) do
		...
		NotifyWithinDistanceAny({ Id = enemy.ObjectId, DestinationNames = enemy.TargetGroups, Distance = weaponAIData.AIAttackDistance, ScaleY = 0.5, MaxZ = weaponAIData.MaxVictimZ, Notify = enemy.AINotifyName })
		waitUntil( enemy.AINotifyName )
		local nearbyTargetId = NotifyResultsTable[enemy.AINotifyName]
	...
	if targetId ~= nil and targetId ~= 0 then
		...
		while not attackSuccess do
			...
			if weaponAIData.ForcedEarlyExit then
				if weaponAIData.AIChargeTargetMarker ~= nil then
...
function MoveWithinRange( enemy, targetId, weaponAIData )
	...
	NotifyWithinDistance({ Id = enemy.ObjectId, DestinationIds = targetId, Distance = attackDistance,
...
function AttackOnce( enemy, currentRun, targetId, weaponAIData, animationObstacleId )
	...
	if weaponAIData.AITrackTargetDuringCharge then
		Track({ Id = enemy.ObjectId, DestinationId = targetId })
	end
...
function TheseusChariotAIMovement( enemy, currentRun, weaponAIData )
	...
	while pathIndex <= #pathIds + 1 do
		...
		if targetId ~= nil and targetId ~= 0 then
			local timedOut = true
			while timedOut and IsAIActive(enemy, currentRun) do
				timedOut = MoveWithinRange( enemy, targetId, weaponAIData )
				if timedOut and (weaponAIData.ForcedEarlyExit or enemy.ForcedWeaponInterrupt ~= nil) then
