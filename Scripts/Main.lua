...
function ValidateTypes( table, tableName, depth, trace )
	...
	-- Strip any non-primitive types from tables before saving
	...
	for tableKey, tableValue in pairs( table ) do
		...
		elseif type(tableValue) == "number" or type(tableValue) == "boolean" or type(tableValue) == "string" then
			...
			if tableName ~= nil then
				DebugPrint({ Text = "Table: "..tableName, LogOnly = true })
			end
			DebugPrint({ Text = "Stripping non-primitive: "..tableKey.." = "..tostring(tableValue) })
			DebugAssert({ Condition = false, Text = "Stripping non-primitive: "..tableKey.." = "..tostring(tableValue) })
		end
	end
