function StartRoomPresentation( currentRun, currentRoom, metaPointsAwarded )
	...
	if AsphodelBoatSoundId ~= nil and currentRoom.EntranceFunctionName ~= "AsphodelEnterRoomPresentation" then
		DebugPrint({ Text="STOP" })
...
function WrappingPostCombatReloadPresentation( eventSource )
	...
	Destroy({ Name = encounter.WrappingData.StartingGroupName })
end
...
function DeathPresentation( currentRun, killer, killingUnitWeapon )
	...
	if CurrentRun.Cleared then
		thread( DisplayLocationText, nil, { Text = "OutroDeathMessage", Delay = 0.95, Color = Color.Red, Layer = "Overlay", AnimationName = "LocationTextBGDeath", AnimationOutName = "LocationTextBGDeathOut", ThreadName = "Outro" } )
	else
	...
	SetThingProperty({ Property = "Graphic", Value = "ZagreusDeathFullscreen", DestinationId = currentRun.Hero.ObjectId })
...
function SurfaceDeathPresentation( currentRun )
	...
	thread( DisplayLocationText, nil, { Text = "OutroDeathMessageAlt", Delay = 0.95, Color = Color.Red, Layer = "Overlay", AnimationName = "LocationTextBGDeath", AnimationOutName = "LocationTextBGDeathOut" } )
	wait( 0.3 )
	-- Destroy({ Id = zagreusDeathFlash.Id })
...
function EasyModeEnabledPresentation()
	...
	if ConfigOptionCache.EasyMode then
		thread( InCombatTextArgs, { TargetId = CurrentRun.Hero.ObjectId, Text = "EasyModeEnabled", Duration = 1.5, LuaKey = "TempTextData", LuaValue = { Resistance = resistance }} )
	end
end
...
function EasyModeDisabledPresentation()
	...
	if not ConfigOptionCache.EasyMode then
		thread( InCombatTextArgs, { TargetId = CurrentRun.Hero.ObjectId, Text = "EasyModeDisabled", Duration = 1.5, LuaKey = "TempTextData", LuaValue = { Resistance = resistance }} )
	end
end
...
function DimLounge()
	if not loungeDim then
		loungeDim = true
		SetAlpha({ Name = "LoungeDimmer_01", Fraction = 1.0, Duration = 0.3 })
...
function UnDimLounge()
	if loungeDim then
		loungeDim = false
		SetAlpha({ Name = "LoungeDimmer_01", Fraction = 0.0, Duration = 0.3 })
...
function DimTopHall()
	if not topHallDim then
		topHallDim = true
		SetAlpha({ Name = "TopHallDimmer_01", Fraction = 1.0, Duration = 0.3 })
	end
end
...
function UnDimTopHall()
	if topHallDim then
		topHallDim = false
		SetAlpha({ Name = "TopHallDimmer_01", Fraction = 0.0, Duration = 0.3 })
	end
end
...
function DoBiomePresentation()
	if not CurrentRun.CurrentRoom.UseBiomeMap then
		return
	end
...
function CleanupRunMapPresentation()
	...
	Destroy({ Name = "Boxen" })
	Destroy({ Name = "MapBackground" })
	Destroy({ Name = "MapLocales" })
	Destroy({ Name = "MapPin" })
	Destroy({ Name = "RewardPreviews" })
	Destroy({ Id = ScreenAnchors.BiomePresentation })
end
...
function DeathAreaAttackFailPresentation( weaponData, args )
	...
	if CheckCooldown( "AttackCheck", 0.2 ) then
		...
		if CheckCountInWindow( "CannotAttack", 1.0, 3 ) and CheckCooldown( "CannotAttack", 10.0 ) then
			...
			thread( InCombatText, args.triggeredById, "NoWeaponsInTheHouse", 1.35 )
		end
	end
...
function AssistHintPresentation( source, args )
	for traitName, traitDatas in pairs( CurrentRun.Hero.TraitDictionary ) do
		if TraitData[traitName] and TraitData[traitName].Slot == "Assist" and not IsEmpty(traitDatas) and traitDatas[1] and traitDatas[1].AnchorId then
			...
			thread( InCombatTextArgs, { TargetId = traitDatas[1].AnchorId, Text = "AssistAvailableHint", Duration = 1.25, ScreenSpace = true, OffsetX = 220, OffsetY = 0 })
