...
function PulseShrineText( id, text, color )
	ModifyTextBox({ Id = id, Color = Color.White, ScaleTarget = 1.03, ScaleDuration = 0.05, })
	wait( 0.05 )
	ModifyTextBox({ Id = id, Color = color, ScaleTarget = 0.99, ScaleDuration = 0.1, })
	wait( 0.1 )
...
function GiftTrackUnlockedPresentation( entryName )
	...
	for i, ids in pairs( createdIds ) do
		SetAlpha({ Ids = ids, Fraction = 0, Duration = 0 })
		SetAlpha({ Ids = ids, Fraction = 1, Duration = 0.1 })
	...
	waitScreenTime(1)
	for i, ids in pairs( createdIds ) do
		SetAlpha({ Ids = ids, Fraction = 0, Duration = 0.33 })
	end
...
function ChillClearPresentation( victim, victimId )
	if victim == nil or ((not victim.IsDead and victim.Health > 0) or victim.ClearChillOnDeath) then
...
function CannotDecrementMetaUpgradePresentation( costTextboxId )
	...
	if CheckCooldown( "BlockedByHardMode", 1.45 ) then
		thread( InCombatTextArgs, { TargetId = costTextboxId, Text = "BlockedByHardMode", Duration = 1.3, Group = "Overlay", ScreenSpace = true, OffsetX = 765, OffsetY = -120 } )
	end
...
function FirstTimeWeaponPickupPresentation( weaponKit )
	...
	wait( 0.75 )
	Move({ Id = CurrentRun.Hero.ObjectId, Angle = 270, Mode = "Precise" })
...
function RelationshipChangedPresentation( entryName, componentIds, finalGiftLevel )
...
function StartedTextLinesPresentation( source, textLines )
	...
	if CurrentRun.Hero.IsDead and AmbientMusicId ~= nil then
		SetVolume({ Id = AmbientMusicId, Value = 0.5, Duration = 0.25 })
	end
...
function FinishedTextLinesPresentation( source, textLines )
	...
	if CurrentRun.Hero.IsDead and AmbientMusicId ~= nil then
		SetVolume({ Id = AmbientMusicId, Value = 1.0, Duration = 0.25 })
	end
...
function HypnosReturnToSleep( source, args )
	...
	while true do
		...
		NotifyWithinDistanceAny({ Id = CurrentRun.Hero.ObjectId, DestinationId = source.ObjectId, Distance = args.InsideDistance, Notify = notifyName })
		waitUntil( notifyName )
