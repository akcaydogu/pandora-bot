...
function CreateSellButtons()
	...
	for itemIndex, sellData in pairs( sellList ) do
		...
		if itemData ~= nil then
			...
			CreateTextBox(MergeTables(LocalizationData.SellTraitScripts.ShopButton,{
				Id = components[purchaseButtonKey].Id, Text = "Sell_ItemCost", TextSymbolScale = 0.6, LuaKey = "TempTextData", LuaValue = { Amount = sellData.Value }, FontSize = 24,
				OffsetY = 60,
				OffsetX = 430,
				Color = Color.Gold,
				Justification = "RIGHT",
...
function HandleSellChoiceSelection( screen, button )
	...
	if button.Rarity == "Legendary" then
